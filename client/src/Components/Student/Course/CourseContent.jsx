...
const CourseDetails = () => {
  ...
  const getCourse = async () => {
    const data = await GET(`${process.env.REACT_APP_FRONTEND_URL}/getcourse/${_id}`);
    setTemp(data);
  };
  ...
  const handleBuyClick = async (item) => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_FRONTEND_URL}/create-checkout-session`,
          ...
          <div className="sm:flex h-full md:h-[90vh] w-full border relative">
            ...
            <div className=" h-full bg-gray-100 sm:pr-0 sm:pb-0 flex-1 overflow-y-scroll">
              ...
              <div className="p-4 gap-1 mx-2 bg-white relative ">
                ...
                >
                  <source
                    src={`${process.env.REACT_APP_FRONTEND_URL}${item.videoPath}`}
                    type="video/mp4"
                  />

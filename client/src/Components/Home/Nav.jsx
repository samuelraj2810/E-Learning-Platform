      ...
      <div className="capitalize tracking-wider w-full p-2 md:flex flex-col md:flex-row md:justify-between items-center hidden">
        {updatedNavList.map((each) => (
          <Link
            ...
            to={each.to}
            key={each.id}
            className={`hover:text-PrimaryDark p-2 rounded scale-100 transition-all h-full flex items-center hover:drop-shadow duration-500 ${
              each.id === menuId && " text-PrimaryDark"
            }`}
          >
            {each.title}
          </Link>
